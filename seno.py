...
def seno(numeros_inteiros, resp,erro,pergunta,page):
    if len(numeros_inteiros) == 1:
        # Calcula o angulo em radiandos e depois calcula o seno
        seno = round(math.sin(math.radians(numeros_inteiros[0])), 3)
        sen = str(seno).replace('.',',')
        numeros_inteiros_local_0 = str(numeros_inteiros[0]).replace('.',',')
        conteudo_1 = f'ok, vamos calcular o seno do ângulo {numeros_inteiros_local_0}°\n'
        conteudo_3 = f'O seno do ângulo {numeros_inteiros_local_0}° é {sen}.'
